...
import ctypes
import threading
from scapy.all import IP, Ether, IPv6, TCP, UDP, ICMP, ARP, sniff, conf, Raw, get_if_hwaddr
            ...
            if len(self.FILTERED_ACTIVITIES[category]['activities']) > 20:
                ...
                if self.FILTERED_ACTIVITIES[category]['flag']:
                    self.FILTERED_ACTIVITIES[category]['attacker-mac'] = list(
                        set([i[4] for i in self.FILTERED_ACTIVITIES[category]['activities']]))
                else:
                    self.FILTERED_ACTIVITIES[category]['attacker-mac'] = []
        ...
        if ICMP in pkt:
            if src_ip == self.MY_IP and src_mac != self.MY_MAC:
                self.FILTERED_ACTIVITIES['ICMP-SMURF']['activities'].append([
                                                                            pkt.time, protocol, src_ip, dst_ip, src_mac, dst_mac, src_port, dst_port, load_len, attack_type, len(pkt)])
